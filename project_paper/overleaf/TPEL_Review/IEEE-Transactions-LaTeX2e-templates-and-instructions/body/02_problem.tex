...
In RL, value-based methods represent a class of algorithms that focus on estimating the value of states or state-action pairs to guide optimal decision-making. The core idea behind these methods is to evaluate each state or action based on its expected return, using these value estimates to formulate policies that maximize cumulative rewards over time. The value function, such as the state-value function \(V(s)\) or action-value function \(Q(s,a)\), plays a central role in these methods, as it allows the agent to make decisions aimed at optimizing long-term reward.

These methods are particularly effective in environments where the state-action space is well-defined and stable. Value-based methods have become foundational in RL, providing efficient and interpretable solutions for control tasks. In this section, we first delve into Q-learning \cite{watkins1992q}, which serves as the bedrock for many value-based methods. We then examine SARSA \cite{rummery1994line}, a variant of Q-learning that offers a different perspective on managing state-action pairs. Next, we explore the advances brought by Deep Q-Networks (DQN) \cite{van2016deep}, which incorporate deep learning techniques to improve performance in large state-action spaces. Following this, we discuss Double DQN (DDQN) \cite{wang2016dueling}, which addresses overestimation biases present in DQN. Finally, we explore various other variants that have been developed to enhance the robustness, stability, and applicability of these methods in more complex and dynamic environments. Each of these algorithms offers distinct advantages in solving reinforcement learning problems, particularly in domains such as power electronics, where precise decision-making and effective system control are paramount.
...
Despite its limitations, Q-learning has been successfully adapted for various PECs applications, particularly in renewable energy systems like MPPT \cite{wei2015reinforcement}. In Wind Energy Conversion Systems (WECs), Q-learning-based MPPT improves tracking speed, reduces sensor dependence, and boosts energy output by 5.6\% compared to traditional methods like Perturb and Observe (P\&O). Similarly, in photovoltaic systems \cite{chou2019maximum}, the h-POQL algorithm enhances tracking efficiency by 90\%, achieving 99.2\% tracking efficiency under rapid irradiation changes. For dual-active-bridge (DAB) converters \cite{tang2020reinforcement}, Q-learning optimizes triple-phase-shift angles, reducing current stress and ensuring zero-voltage-switching (ZVS), improving efficiency by 29.4\% and reducing peak current by 25\%. In DC microgrids \cite{chaturvedi2023reinforcement}, Q-learning-based integrated control reduces transmission losses by 21\%, ensuring stability during load sharing. In Hybrid Renewable Energy Systems (HREs) \cite{zhang2020virtual}, Q-learning optimizes virtual inertia in Virtual Synchronous Generator (VSG) control, reducing frequency and power oscillations, improving damping performance, and enhancing grid stability. Additionally, Q-learning integrated with Model Predictive Control (MPC) improves DC-DC boost converter control, reducing transient oscillations by 62.5\% and settling time by 40\% \cite{marahatta2022model}.
...
In conclusion, Q-learning proves to be an effective and flexible challenges remain in terms of scalability for high-dimensional systems, Q-learning's application in tasks such as MPPT, system optimization, and energy management demonstrates its substantial contributions to improving system stability, reducing power losses, and enhancing efficiency. Future research could explore hybrid models integrating Q-learning with other advanced algorithms to extend its applicability in more complex PECs.

\subsubsection{State–Action–Reward–State–Action (SARSA)}

SARSA (State–Action–Reward–State–Action) is an on-policy RL algorithm that updates Q-values based on the action taken in the next state, rather than always selecting the greedy action. This inherent difference makes SARSA more conservative in its policy updates compared to Q-learning, making it particularly beneficial in safety-critical applications, such as fault-tolerant control in power converters. While Q-learning is widely used for general optimization tasks, SARSA's on-policy nature ensures more cautious adjustments, reducing the risk of instability during fault conditions, which is crucial for maintaining the stability of PECs.

Similar to Q-learning, SARSA also faces scalability limitations in high-dimensional systems. However, SARSA's more conservative nature can be advantageous when applied to complex systems that require stable and adaptive decision-making processes. For instance, SARSA has been effectively applied in multi-energy microgrids to optimize energy management and demand response \cite{mughees2025optimizing}. In this context, SARSA, when combined with differential evolution and deep learning-based forecasting, enables real-time adaptation to varying demand and renewable energy generation profiles. This method addresses the challenges of fluctuating energy sources and unpredictable demand. SARSA has demonstrated significant improvements, achieving a 92.4\% reduction in energy costs and a 48.2\% increase in potential revenue, making it an effective tool for optimizing both economic performance and system stability.
...
Deep Q-Network (DQN) extends value-based RL to high-dimensional control problems by approximating the Q-function using deep neural networks. This allows DQN to handle complex power electronics systems such as modular multilevel converters (MMCs), inverter-based grid interfaces, and DC-DC converters \cite{van2016deep}. Despite its scalability, DQN may overestimate Q-values due to the maximization operator in target calculation, potentially degrading training stability and leading to suboptimal performance.

DQN has been successfully applied in regulating voltage in DC-DC buck converters feeding Constant Power Loads (CPLs), significantly enhancing dynamic performance in DC microgrids. The algorithm adjusts the duty cycle in real time, reducing settling time by 90\% (1.79 ms vs. 18 ms) and limiting voltage deviation to less than 0.2\% (0.2 V vs. 2.02 V), outperforming traditional PI control with a 10$\times$ faster dynamic response and ensuring large-signal stability. In MMCs, DQN-based modulation effectively balances capacitor voltage and mitigates thermal stress, improving current capability by 20\% and extending the lifespan of semiconductor devices. It reduces thermal imbalance by 66.7\% and maintains capacitor voltage fluctuation within a 3\% margin. For microgrids, DQN optimizes real-time energy scheduling, decreasing electricity purchase costs by 25\% and enhancing renewable energy utilization by 35\%, resulting in a 20\% reduction in operational costs. Similarly, in hybrid AC/DC microgrids, DQN reduces power losses by 14.4\% and extends the lifespan of interlinking converters by 30\%. In grid-supportive control tasks, DQN has been applied to voltage regulation at the point of common coupling, replacing traditional PID control. This approach reduces current tracking error by 55.56\%, eliminates steady-state error, and enhances transient response to sub-cycle times ($<$0.1 s).

DQN has been applied successfully in regulating voltage in DC-DC buck converters feeding CPLs, improving dynamic performance in DC microgrids. The algorithm adapts to voltage fluctuations by adjusting the duty cycle in real-time, reducing settling time by 90\% (1.79 ms vs. 18 ms) and limiting voltage deviation to less than 0.2\% (0.2 V vs. 2.02 V). This model-free approach outperforms traditional PI control, achieving a 10$\times$ faster dynamic response and ensuring large-signal stability.
...
Double Deep Q-Network (DDQN) addresses the overestimation bias inherent in traditional DQN by decoupling action selection and evaluation through separate online and target networks \cite{wang2016dueling}. This decoupling reduces Q-value overestimation, enhances convergence stability, and improves decision reliability, making DDQN particularly suitable for dynamic, nonlinear, and safety-critical control scenarios in power electronics and energy systems.

DDQN has been successfully applied to stabilize voltage in DC microgrids with CPLs, significantly enhancing dynamic performance during high-load disturbances. The algorithm adjusts the duty cycle in real-time, reducing voltage deviation by 83.3\% (from ±15\% to ±2.5\%) and achieving 0\% steady-state error under 10kW CPL steps. It enables sub-cycle stabilization (<1ms) with a 100\% success rate, outperforming traditional controllers in both speed and accuracy. DDQN optimizes battery balancing and charging safety in electric vehicle fast charging, improving efficiency and reducing charging time. The algorithm decreases charging time by 34 seconds compared to conventional methods, increases the terminal state of charge by 5\%, and reduces SOC imbalance to ±0.5\%, ensuring better battery health and faster charging while mitigating the "cask effect" caused by cell inconsistencies.
...
%n addition to energy management, DDQN-based methods have been utilized to enhance the resilience and security of smart grid systems, particularly in addressing vulnerabilities like eavesdropping and signal blocking \cite{lian2023secure}. Multi-agent DRL frameworks optimize communication protocols, such as mmWave relay selection and directional beamforming, which are critical for smart grids using power electronic converters. This improves security by reducing eavesdropping vulnerability by 85\% and achieving 100\% secret connection probability (SCP), outperforming random selection by 62\% in secrecy rate.
...
Proximal Policy Optimization (PPO) is a reinforcement learning algorithm that balances training stability and computational efficiency by approximating the trust region constraint with a clipped surrogate objective \cite{schulman2017proximal}. PPO's robustness and scalability have made it a popular choice for optimizing PECs and broader energy systems, where dynamic conditions and nonlinear behavior present significant challenges.

PPO has been effectively applied in various PECs to enhance performance and stability. In PV systems, PPO has optimized MPPT under partial shading conditions, using a hybrid Crayfish Optimization Algorithm (COA)-PPO controller to improve DC-DC boost/buck-boost converter efficiency, achieving 95.14\% efficiency compared to 94.24\% with traditional P\&O-PI methods. Additionally, PPO has been deployed in photovoltaic-battery storage systems to optimize energy management and enhance voltage regulation. By optimizing the parameters of Buck and Boost converters in real-time through closed-loop interactions with Simulink models, this approach reduces optimization time by 80\% and achieves 97.7\% efficiency, matching brute-force optimization results.

Furthermore, PPO has been used to stabilize DC-DC converters under CPLs in DC microgrids, which often cause destabilizing negative impedance. By optimizing the coefficients of a Model-Free Sliding Mode Controller (SMC), PPO reduces settling time by 77\% (from 52 ms to 12 ms) and reduces voltage ripple by over 60\%, ensuring large-signal stability during transients. Moreover, PPO, when combined with a Lyapunov-guided framework, has optimized converter performance in buck converters, reducing total harmonic distortion (THD) by 21.6\% and improving dynamic response, ensuring system stability and enhancing steady-state accuracy. These advancements in PPO-based control techniques significantly enhance the performance, efficiency, and stability of power electronic converters across various applications.
...
These diverse applications demonstrate PPO's versatility and effectiveness in optimizing PECs. Its ability to handle dynamic conditions, optimize system parameters, and ensure stability in real-time makes it a powerful tool for enhancing the performance and reliability of modern energy systems.
...
The Deep Deterministic Policy Gradient (DDPG) algorithm is a model-free, off-policy actor-critic method designed for continuous control tasks, making it particularly suitable for managing the complex dynamics of PECs \cite{lillicrap2015continuous}. DDPG utilizes separate actor and critic networks, each with a target counterpart, and employs deterministic policy gradients for efficient learning. The integration of experience replay and soft updates further enhances sample efficiency, making DDPG highly effective for high-dimensional control tasks. However, it is sensitive to hyperparameters and can suffer from Q-value overestimation, which may hinder performance in some complex applications.

DDPG has been effectively applied across various PECs to optimize performance and enhance stability. In hybrid energy storage systems for microgrid applications, DDPG has been combined with deadbeat control to mitigate nonlinear power loss and model parameter mismatch, resulting in a 34.24–44.44\% reduction in bus voltage spikes and a 16.66–40.00\% reduction in settling time, while maintaining steady-state error within 1\%. In MMCs, DDPG optimizes harmonic RMS values, reducing THD by 55.4\% compared to selective harmonic elimination and 36.7\% compared to nearest-level modulation, while maintaining capacitor voltage balance within 2.6\%. For multi-DC terminal PEC converters in 5G commercial buildings, DDPG enhances voltage stability and reduces voltage spikes by 44.44\%, achieving a 2.6\% THD for grid current, which surpasses IEEE-1547 standards.

Moreover, DDPG has been successfully employed in SIMO DC-DC converters to optimize duty cycles for shared inductor control, eliminating cross-regulation and reducing output voltage ripple by 50\%, achieving a peak efficiency of 90.4\%. In finite-set MPC applications, DDPG automates weighting factor optimization, resulting in a 34.1\% reduction in THD and an 8.1\% reduction in switching frequency, while eliminating the need for model reliance. These advancements highlight DDPG's significant role in improving the efficiency, stability, and overall performance of PECs in diverse applications.
...
TD3 has demonstrated significant potential in optimizing PECs across various applications. In low-inertia microgrids, TD3 has been used to optimize virtual inertia emulation and inverter control, reducing frequency deviation by 56.6\% (from ±0.25 Hz to ±0.06 Hz) and cutting transient recovery time by 40\% during load steps. This model-free approach eliminates manual PI tuning, ensuring robust frequency stability during grid transitions and faults. In fuel cell hybrid railway vehicles, TD3 coordinates fuel cell DC/DC converters and battery current controllers, reducing fuel cell voltage degradation by 50.6\% (from 13.7~\micro V to 6.9~\micro V) and operational costs by 28\%, while maintaining battery state of charge within ±0.01 of reference during disturbances caused by passenger loads.

Moreover, in urban railway hybrid energy storage systems, TD3 optimizes dynamic power allocation to mitigate traction voltage fluctuations, reducing voltage variations by 93\% (from 1046–1887 V to 1461–1520 V) and boosting energy-saving efficiency to 65.6\%. It also reduces training convergence time by 29\% (50 episodes vs. 70 for DDPG), outperforming traditional methods and enhancing overall system stability. These advancements highlight TD3's capability to improve the performance and efficiency of PECs in diverse applications, ensuring enhanced stability, reduced losses, and optimized power management.
...
In summary, TD3's ability to handle continuous action spaces and adapt to dynamic, uncertain environments makes it an invaluable tool for optimizing PECs. From energy management in fuel cell vehicles to voltage regulation in photovoltaic-rich grids and microgrid stabilization, TD3 has demonstrated significant effectiveness in solving nonlinear control tasks within power electronics. As energy systems evolve, TD3 is poised to play a critical role in advancing adaptive, intelligent control strategies for next-generation power systems.

\subsubsection{Soft Actor–Critic (SAC)}

The Soft Actor-Critic (SAC) algorithm integrates entropy regularization into the actor-critic framework, optimizing a stochastic policy that balances expected return and entropy, thereby enhancing exploration and robustness in dynamic, uncertain environments \cite{haarnoja2018soft}. SAC's use of multiple Q-networks, stochastic policies, and soft updates ensures impressive sample efficiency and stability, making it particularly effective in continuous control tasks. However, SAC's relatively high computational cost may pose challenges for real-time deployment in embedded PECs. Despite this, SAC has shown exceptional performance in optimizing a variety of energy management and control tasks, particularly in applications involving hybrid electric vehicles, photovoltaic systems, and microgrids.

SAC has been widely applied to optimize PECs across various fields, significantly enhancing performance and stability. In hybrid electric buses, SAC optimizes energy distribution, improving battery life, managing thermal stress, and enhancing the efficiency of DC-DC converters, thereby reducing thermal safety issues and prolonging the lifespan of power electronics. In integrated energy systems, SAC has been used to optimize switching actions in bidirectional DC/AC inverters, improving power conversion efficiency by reducing transient response time by 40\% and cutting voltage ripple by 63\%, compared to traditional PI control.

SAC also plays a crucial role in microgrid frequency control, optimizing virtual inertia and damping in energy storage systems to stabilize the grid during power fluctuations. This is particularly important for PECs like inverters and DC-DC converters, which regulate power flow and ensure grid stability, especially with the integration of intermittent renewable energy sources. Additionally, SAC has optimized PI-IR controllers for energy storage-integrated voltage source converters, reducing settling time by 40\% and cutting DC-link voltage ripple by 71\%, improving system stability and steady-state performance.

In electric vehicle (EV) charging stations, SAC optimizes charging schedules and real-time power conversion, improving the efficiency of DC-DC converters and reducing voltage deviations during high-power transients. This approach also eliminates transformer overload, improving system efficiency. Furthermore, SAC addresses grid impedance fluctuations in low-X/R-ratio microgrids, optimizing power electronic converters to reduce power oscillations by 50\%, improve power decoupling, and enhance system stability. These applications demonstrate SAC's broad capability to enhance PECs across various systems, ensuring improved efficiency, stability, and longevity.
...
SAC optimizes energy management and control in PECs across various applications, enhancing efficiency, stability, and longevity. It improves DC-DC converter performance, reduces voltage ripple, stabilizes microgrid frequency, and optimizes charging in electric vehicles, despite its high computational cost for real-time deployment.
...
A3C has been effectively applied to optimize PECs across various applications. In shipboard integrated power systems, A3C optimizes supercapacitor charging thresholds and power distribution, reducing charging time by 40\% (from 10s to 6s) and limiting frequency deviation to <0.5 Hz, improving system stability during transients. In low-inertia microgrids, A3C optimizes virtual inertia emulation and inverter control, reducing frequency deviation by 56.6\% (from ±0.25 Hz to ±0.06 Hz) and cutting transient recovery time by 40\%, ensuring robust frequency stability during grid transitions and faults.

In hybrid energy storage systems for urban railways, A3C coordinates power electronic converters to mitigate voltage fluctuations, reducing fluctuations by 93\% (from 1046–1887 V to 1461–1520 V) and boosting energy-saving efficiency to 65.6\%, while shortening training convergence time by 29\%. A3C also optimizes battery management and energy distribution in multi-source energy systems like EVs and integrated energy subsystems. In EVs, A3C reduces state-of-charge convergence time by 58.5\% (from 1.52s to 0.63s) and limits DC voltage deviation to ±0.5\% during 150A transients. In IES, A3C coordinates supply and demand-side resources, reducing active power tracking error by 42.7\% and limiting voltage deviations at the point of common coupling to <1.2\% during 150-kW transients. These applications demonstrate A3C's broad capability to optimize PECs, enhancing efficiency, stability, and performance across diverse systems.

%A3C has been applied to various power electronic converter, optimizing energy management, voltage control, and multi-source coordination in dynamic environments. In shipboard integrated power systems (SIPS), where voltage and frequency instability (>±5\% deviation) arises due to pulsed power loads (PPLs), A3C optimizes supercapacitor (SC) charging thresholds and power distribution, improving stability during transients. This reduces SC charging time by 40\% (from 10s to 6s) and limits frequency deviation to <0.5 Hz, compared to 1.5 Hz with traditional PI control \cite{zhang2022optimal}. Similarly, in low-inertia microgrids, A3C optimizes virtual inertia emulation and inverter control, reducing frequency deviation by 56.6\% (from ±0.25 Hz to ±0.06 Hz) and cutting transient recovery time by 40\% during load steps, ensuring robust frequency stability under grid transitions and faults \cite{egbomwan2022twin}.
...
In summary, A3C's ability to handle continuous action spaces and adapt to dynamic, uncertain environments makes it an invaluable tool for optimizing PECs. From improving battery management in electric vehicles to enhancing the scheduling of microgrids and increasing the efficiency of wind energy systems, A3C offers powerful solutions for the optimal operation of power electronic converters. Its real-time adaptability ensures robust performance, making it a crucial component in the advancement of modern energy systems.

\subsubsection{Other Variants}

In addition to the core actor-critic frameworks, various enhancements have been proposed to improve convergence speed, sample efficiency, and training stability. Notable examples include synchronous updates (e.g., A2C) \cite{mnih2016asynchronous}, experience replay with importance sampling (e.g., ACER) \cite{wang2016sample}, second-order optimization (e.g., ACKTR) \cite{wu2017scalable}, and distributional reinforcement learning (e.g., D4PG) \cite{bellemare2017distributional}. These variations introduce algorithmic refinements to actor-critic methods but remain secondary in PECs applications, where actor-critic paradigms dominate due to their balance between learning stability, continuous control capability, and implementation scalability. These factors are critical for managing complexities in power systems, such as voltage regulation, grid stability, and battery management.

A prominent application of actor-critic methods is in optimizing volt-VAR droop curves for solar photovoltaic smart inverters in active distribution networks. This approach improves voltage regulation, reduces losses under overload conditions, and ensures compliance with IEEE 1547-2018 standards. The barrier-filtered A2C algorithm dynamically optimizes QV-droop curves for smart inverters, reducing inductor current ripple by 71\%, lowering IGBT junction temperature to ($\Delta T$) to $\leq$18\,$^\circ$C (from 35°C), and decreasing switching losses by 22.7\%, all while maintaining compliance with industry standards.
...
Actor-Critic Methods in reinforcement learning, including DDPG, TD3, SAC, and A3C, offer significant advantages for optimizing control tasks in environments with continuous action spaces, such as PECs. These methods combine the benefits of both value-based and policy-based approaches, enabling efficient learning and adaptation in complex, dynamic environments. While they have proven highly effective in applications such as voltage regulation, energy management, and system stabilization, they also face challenges related to stability, training efficiency, and the handling of high-dimensional action spaces. Among these methods, DDPG and its variants, including TD3 and SAC, have shown impressive performance in improving sample efficiency and stability, making them particularly suitable for high-dimensional control tasks. SAC, in particular, offers robust exploration capabilities due to its entropy regularization mechanism, though its computational cost remains a challenge for real-time applications in resource-constrained systems. A3C, with its asynchronous learning structure, accelerates training speed and enhances exploration, but its inherent instability in real-time, safety-critical applications needs careful management. Future research should focus on improving the sample efficiency of these algorithms, reducing their computational demands, and addressing the stability challenges they pose in dynamic environments. Furthermore, enhancing the scalability of these methods and ensuring their robustness in real-time applications will be critical for their broader adoption in power electronics. 

\subsection{Multi-Agent}
\label{sec:ma_methods}

The emergence of Multi-Agent Reinforcement Learning (MARL) methods has fundamentally transformed the approach to decentralized control in systems where multiple agents must collaborate and autonomously make decisions. These methods build on traditional reinforcement learning by allowing agents to interact with each other, learning not only from the environment but also from the actions of their peers. This interaction is particularly beneficial in complex, distributed systems like PECs, where numerous agents, such as inverters and energy storage systems, need to work in coordination to optimize performance while respecting system constraints.

Among the key MARL algorithms, Multi-Agent DDPG (MADDPG) \cite{lowe2017multi} extends the DDPG algorithm to multi-agent settings, allowing agents to share information through a centralized critic while learning coordinated policies. Multi-Agent PPO (MAPPO) \cite{yu2022surprising} builds upon PPO, bringing its advantages to multi-agent environments and offering a scalable and efficient approach to policy optimization. Multi-Agent TD3 (MATD3) \cite{ackermann2019reducing} extends the TD3 algorithm to multi-agent scenarios, maintaining the stability improvements of TD3 while managing coordination in real-time. Multi-Agent SAC (MASAC) \cite {kuba2021trust} incorporates the entropy regularization feature of SAC into multi-agent settings, enhancing exploration and robustness for coordinated control in dynamic environments. Additionally, other notable methods such as Multi-Agent Actor-Critic (COMA) \cite{foerster2018counterfactual} and Monotonic Value Function Factorization (QMIX) \cite{rashid2020monotonic} have emerged, each offering unique advantages for addressing the challenges of coordination and scalability in multi-agent systems. COMA uses counterfactual baselines to improve credit assignment in multi-agent settings, while QMIX applies monotonic value function factorization to ensure global optimality in decentralized training. Collectively, these algorithms form a comprehensive toolkit for solving complex coordination challenges in PECs, offering effective solutions for tasks such as voltage regulation, energy management, and optimization in distributed systems.

\subsubsection{MADDPG}

MADDPG is a powerful MARL algorithm that extends the traditional DDPG to multi-agent settings. By using a centralized critic with decentralized actors, MADDPG enables agents to learn optimal policies while accounting for the actions of other agents in the system. This makes MADDPG particularly useful in PECs, where multiple agents—such as inverters, energy storage systems, and distributed energy resources—must coordinate in real-time to optimize system performance in dynamic and uncertain environments.

MADDPG has demonstrated significant potential in optimizing PECs across various applications. In distribution networks, MADDPG optimizes voltage regulation by coordinating voltage control devices such as on-load tap changers and capacitor banks, effectively reducing maximum voltage deviations by 100\%, eliminating voltage violation time, and ensuring stable grid operation. It also coordinates photovoltaic inverters as fast-reactive power sources, achieving near-optimal performance with minimal power loss. In energy internet systems, MADDPG enhances reactive power compensation by coordinating distributed devices like Static VAR Generators, improving voltage stability and reducing reactive power overshoot by 44.4\%.

In energy storage systems for urban rail systems, MADDPG optimizes energy distribution, increasing energy feedback by 3.8\% and energy savings by 2\%, while reducing braking resistor loss by 44.4\%. Additionally, MADDPG effectively addresses renewable energy volatility in grids with high renewable penetration, coordinating reactive power regulation to maintain voltage stability during grid disturbances and ensuring fast, reliable system stability. These applications highlight MADDPG's ability to optimize PECs, improving efficiency, stability, and performance in dynamic and fluctuating environments.
...
Overall, MADDPG is a key algorithm in PECs, offering innovative solutions for distributed control, voltage regulation, and energy management in complex, decentralized environments. Its ability to enable real-time coordination between multiple agents and optimize system-wide performance makes it invaluable in modern power systems, especially those that integrate renewable energy and distributed resources. MADDPG's versatility in addressing voltage fluctuations, optimizing reactive power, and managing energy distribution positions it as a critical tool for advancing the reliability and efficiency of PECs in future energy systems.

\subsubsection{MAPPO}

The Multi-Agent Proximal Policy Optimization (MAPPO) algorithm is an advanced extension of PPO designed specifically for multi-agent environments. By incorporating a centralized critic with decentralized actors, MAPPO enables multi-agent coordination, making it well-suited for decentralized systems like PECs. In PECs, multiple agents—such as inverters, energy storage systems, and renewable energy sources—must work together in real-time to optimize performance in dynamic and uncertain environments. MAPPO's ability to efficiently handle this coordination is one of its key strengths.

MAPPO has been successfully applied to optimize PECs across various applications. In VSC-HVDC systems, MAPPO enhances voltage regulation and line flow management, particularly in grids with high renewable energy penetration. It ensures real-time regulation under fluctuating conditions, reducing voltage deviations to <0.05 p.u. and achieving a 2.8 ms response time for emergency control, 99.9\% faster than traditional methods. In photovoltaic systems, MAPPO optimizes volt-var control for PV inverters, addressing voltage imbalances caused by high single-phase PV penetration. It dynamically adjusts the volt-var curve in real-time, reducing voltage unbalance by 10.87\% in IEEE 13-bus systems, outperforming traditional fixed droop inverters.

Additionally, MAPPO plays a key role in energy storage systems, where it optimizes energy storage operation, mitigating wind power fluctuations and improving grid stability. By integrating wavelet packet power decomposition, MAPPO reduces grid-connected power fluctuations by 71.46\% and lowers operational costs by 18.31\% compared to baseline PPO. These applications highlight MAPPO's significant impact on optimizing PECs, improving system efficiency, stability, and performance in dynamic and fluctuating environments.
...
Overall, MAPPO has shown transformative potential in PECs. By enabling multi-agent coordination, MAPPO excels in real-time control and optimization across applications such as voltage regulation, reactive power management, and renewable energy fluctuation mitigation. Its scalability and efficiency make it a powerful tool for modern PECs, particularly as energy systems evolve to integrate more distributed energy resources and renewable sources. MAPPO's adaptability to dynamic environments and ability to optimize energy management will continue to play a pivotal role in the advancement of PECs in the future.

\subsubsection{MATD3}

The Multi-Agent Twin Delayed Deep Deterministic Policy Gradient (MATD3) algorithm, an extension of the TD3 framework, has demonstrated significant potential in addressing the challenges of multi-agent coordination in PECs. MATD3 combines the stability and performance improvements of TD3 with a multi-agent setup, making it particularly suitable for dynamic and decentralized systems, such as those found in modern power grids. MATD3 addresses the coordination of multiple agents in high-dimensional and continuous action spaces, offering solutions for complex tasks like voltage regulation, energy management, fault resilience, and optimal energy trading in decentralized power systems.

MATD3 has proven highly effective in optimizing PECs across various applications. In grid-tied photovoltaic systems, MATD3 optimizes fault resilience in parallel inverters, reducing transition time to fault-tolerant operation to just 6 ms, ensuring stable grid operation during renewable energy fluctuations. This significantly improves system reliability compared to traditional methods. In DC microgrids, MATD3 addresses real-time generation control, reducing transient generation costs by 62.5\% and eliminating voltage violations, maintaining all bus voltages within ±5\% bounds while reducing operational costs.

Furthermore, MATD3 enhances the performance of multi-energy microgrids, where it optimizes peer-to-peer energy trading and converter actions, including electrolyzers and fuel cells. By coordinating distributed agents, MATD3 reduces costs by up to 18.2\% for residential, 16.5\% for commercial, and 18.1\% for industrial microgrids, while accelerating convergence by 40\%. These applications highlight MATD3's ability to improve the efficiency, stability, and performance of PECs in dynamic environments, ensuring better grid integration and energy management.
...
The Multi-Agent Soft Actor-Critic (MASAC) algorithm is a powerful extension of the SAC framework, tailored for multi-agent environments. By integrating multiple agents into a shared learning space, MASAC enhances coordination, exploration, robustness, and stability, making it highly suitable for decentralized systems such as PECs. In PECs, where multiple distributed agents—such as inverters, energy storage systems, and renewable energy resources—need to collaborate in real-time to optimize system performance, MASAC provides a reliable solution for handling dynamic and uncertain environments.

MASAC has proven highly effective in optimizing PECs across various applications. In photovoltaic-rich grids, MASAC coordinates energy storage systems to provide real-time voltage support, eliminating voltage violations and reducing power losses by 24.5\%. It ensures sub-cycle transient response (<0.1s) and maintains voltage stability within ±0.5\% deviation. In active distribution networks, especially with high renewable energy penetration, MASAC improves voltage control by integrating hydrogen energy storage systems, reducing voltage deviations to ±0.01 p.u. and cutting power losses by 24.5\%, with sub-millisecond response times for real-time reactive power regulation.

In multi-microgrid systems, MASAC addresses challenges like nonlinear equipment characteristics and renewable energy uncertainties, optimizing energy distribution and reducing operating costs by 34.55\%. It also cuts carbon emissions by 13.29\% and ensures voltage stability within ±0.01 p.u. during fluctuating renewable conditions. In grid-connected battery energy storage systems, MASAC enhances output current sharing and voltage regulation by coordinating DAB-based converters, improving dynamic performance, reducing current-sharing MSE by 60.8\%, and cutting transient response time by 48\%. These applications demonstrate MASAC's ability to improve efficiency, stability, and performance in PECs across diverse energy systems.
...
Overall, MASAC has proven to be a transformative tool for optimizing multi-agent coordination in PECs. Its ability to optimize real-time decision-making and adaptive control has significant applications in voltage regulation, energy management, and renewable energy fluctuation mitigation. MASAC's scalability, efficiency, and adaptability position it as a critical technology in modern energy systems, particularly those integrating distributed energy resources and renewable sources. As energy systems continue to evolve, MASAC will play a key role in ensuring the stability, efficiency, and reliability of next-generation power electronics systems.

\subsubsection{other multi-agent methods}

In addition to well-established algorithms like MADDPG, several other MARL methods have been explored to address the complexities of decentralized control and dynamic environments in PECs. Algorithms such as QMIX, MA-D4PG, and CMA-4DPG offer tailored solutions that enhance stability, efficiency, and coordination in real-time, particularly in systems where distributed energy resources and other components need to interact to optimize performance.

In power systems, a MADRL framework combining QMIX and TD3 algorithms, along with graph convolutional networks (GCN), has been applied to optimize real-time frequency and voltage regulation at the Grid Supply Point. This approach addresses the challenges posed by low-inertia dynamics and coupled frequency/voltage oscillations in grids with high distributed energy resources penetration. It achieves sub-second stabilization and reduces voltage deviation to <0.5\%, outperforming traditional methods. In Proton Exchange Membrane Fuel Cells, CMA-4DPG optimizes the coordination between hydrogen valves and DC/DC converters, preventing voltage fluctuations and improving voltage tracking. This method reduces output voltage overshoot by 71\% and settling time by 84\%, compared to traditional PID control. These applications showcase the effectiveness of MARL in enhancing the stability and efficiency of PECs, particularly in dynamic and fluctuating environments.
...
These examples highlight the increasing relevance of MARL in PECs. By enabling decentralized control and enhancing coordination among multiple agents, MARL methods like QMIX, CMA-4DPG, and other approaches address critical challenges such as voltage regulation, frequency control, and energy management in dynamic and distributed environments. The applications in TSO-DSO coordination and PEMFC voltage control illustrate how these methods optimize real-time decision-making, improve system stability, and ensure efficient energy management in modern power grids. As renewable energy integration and distributed control systems continue to evolve, MARL-based approaches will play a pivotal role in advancing intelligent energy management for the next generation of PECs.

\subsubsection{Discussion}

MARL methods, including MADDPG, MAPPO, and MASAC, hold significant potential for optimizing decentralized control in PECs, especially in dynamic environments like grids with high renewable energy penetration. By enabling agent communication, MARL enhances voltage regulation, energy management, and system stability, addressing challenges such as intermittent generation and load fluctuations. However, scalability, stability, and convergence issues remain, particularly in large-scale systems with noisy or limited data. Future research should focus on improving stability, sample efficiency, and computational efficiency for real-time control in resource-constrained systems. Additionally, enhancing communication and coordination between agents is crucial for efficient PECs operation. Frameworks like QMIX and MASAC have made progress, but incorporating advanced techniques such as GCN and prioritized experience replay (PER) could further optimize decision-making and MARL performance in PECs.
...
\subsubsection{Policy gradient and actor-critic frameworks emerged as dominant paradigms post-2017, leveraging computing advancements} PPO (2017) rapidly surpassed value-based methods due to its clipped objective function – balancing sample efficiency with convergence stability (F0-F5). Concurrently, off-policy actor-critic algorithms (DDPG, TD3, SAC) gained traction for continuous control tasks like grid synchronization and MPPT (G3-G6, I2-I5, J3-J5). By 2021, these methodologies collectively accounted for $\>$72\% of PEC-focused DRL implementations (evidenced by node density in Fig. 5(green zone)), driven by GPU-accelerated training and refined neural architectures.

\subsubsection{Multi-agent systems represent the cutting edge (2021–present), addressing distributed control imperatives} The rise of renewable-integrated grids necessitated coordinated control across spatially dispersed converters. Algorithms such as MAPPO (2021), MASAC (2021), and MATD3 (2021) (L0, M1-M3, N0-1) emerged to optimize decentralized decision-making under communication constraints. Their proliferation (67\% of 2021–2025 applications) reflects industry demand for scalable, fault-tolerant control in modular PEC systems. 
