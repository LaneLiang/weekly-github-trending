...
\usepackage{balance}
\begin{document}
\title{Deep Reinforcement Learning Methods Applied to Power Converters: A Review}
\author{Chuanchuan Liang,Xueyong Zhan
...
\begin{abstract}
Power electronic converters (PECs) are crucial components in modern energy systems, especially for renewable energy integration, microgrids, and portable electronics. Due to their high energy efficiency, controllability, and decreasing cost, PECs are pivotal in enabling decentralized, sustainable power generation.However, challenges such as nonlinear dynamics, real-time control limitations, multi-objective design optimization, and uncertain operational conditions hinder their full potential. These challenges often lead to high-dimensional, nonconvex optimization problems, which traditional rule-based or model-driven methods struggle to address.
Deep reinforcement learning (DRL), a model-free approach that integrates deep learning with reinforcement learning, has emerged as a powerful solution for sequential decision-making in complex, uncertain environments. Its ability to learn optimal policies through interaction makes it particularly effective in addressing the diverse challenges throughout the PEC lifecycle. This paper provides a comprehensive, lifecycle-oriented review of DRL applications in PECs, focusing on three primary phases: design optimization, intelligent control, and predictive maintenance (PM). DRL algorithms are categorized into value-based, policy-based, and actor-critic methods, and are critically assessed based on performance metrics, learning efficiency, and implementation challenges. A comparative analysis of these approaches highlights their strengths, limitations, and suitability for various PEC scenarios. By synthesizing current advancements and identifying remaining challenges, this review aims to offer a structured perspective on DRL's role in enabling intelligent, autonomous PECs and to provide insights for future interdisciplinary research in this domain.
...
\IEEEPARstart{O}{VER} the past few decades, artificial intelligence (AI) has undergone significant advancements, establishing itself as one of the most transformative technologies across multiple industries, including image classification and autonomous systems \cite{mnih2015human， 8103164}. Among various AI techniques, deep reinforcement learning (DRL)—which integrates the perceptual power of deep learning with the sequential decision-making framework of reinforcement learning—has emerged as a powerful, model-free paradigm for tackling high-dimensional control 

\begin{figure}[!t]
\centering
\includegraphics[width=0.45\textwidth]{AI_DRL_Publication_Trend_Updated.pdf}
\caption{Annual number of publications on AI in power electronics since 1993. Data were collected from Web of Science using relevant keywords and filtered by journal and year.}
\label{AI_DRL_Publication_Trend_Updated}
\end{figure}

challenges under uncertaint.
DRL's capacity to learn from continuous interaction with dynamic environments has demonstrated substantial potential in power electronic systems (PECs), particularly in areas demanding adaptive and intelligent control strategies. In real-time control applications, DRL has been successfully applied to tasks such as maximum power point tracking (MPPT), inverter regulation, and power quality management \cite{8991860, 10424981, 10197504}. Additionally, DRL has been utilized in converter design for multi-objective optimization, including component sizing and switching frequency \cite{9841555, 9950526}. 
%In the realm of predictive maintenance (PM), DRL has proven effective in fault diagnosis, health prognosis, and predicting the remaining useful life (RUL) of critical components [8], [9], [10].
These advancements position DRL as a promising solution for enabling intelligent, adaptive decision-making throughout the entire lifecycle of PECs. This lifecycle-based approach lays the foundation for next-generation power electronics that are self-aware, resilient, and capable of functioning autonomously in dynamic environments.

Concurrently, the advancement of sensor networks, Internet-of-Things (IoT), edge computing, and digital twins \cite{9841555, 9139976,11005593} has notably enhanced the data availability and system observability of PECs across their entire lifecycle. These technologies facilitate the real-time acquisition of heterogeneous operational data—such as voltage, current and switching states—that are crucial for design optimization, health monitoring and adaptive control [14].This data-rich environment lays a practical foundation for deploying DRL, which relies on continuous interaction with the environment and feedback from multi-source process data [15]. As a result, integrating DRL with data-enabled PEC systems supports intelligent design, real-time control, and predictive maintenance, thereby facilitating a lifecycle-oriented, data-driven approach to power electronics.

Due to the distinctive challenges of power electronic systems—such as rapid control response and sensitive condition monitoring—the application of DRL exhibits domain-specific characteristics that differ from conventional AI deployments. This necessitates a dedicated overview of DRL in power electronics. In this article, relevant applications are categorized into three primary domains: design optimization, real-time control, and predictive maintenance.

Fig \ref{AI_DRL_Publication_Trend_Updated} illustrates the annual publication trends related to DRL applications in PECs across three Of unctional domains: design, control, and maintenance. The data were extracted from Web Of Science including journals—IEEE Transactions on Power Electronics, IEEE Transactions on Industrial Electronics, IEEE ACCESS, IEEE Transactions on Industrial Informatics, and IEEE Transactions On Circuits And Systems II Express Briefs—covering the period from 1993 to 2025(). In total, 573 AI-related publications were identified, of which 58 employed DRL techniques. As shown, the volume of AI applications in PEC has significantly increased over the past decade, with control-oriented studies consistently accounting for the largest share. Starting around 2010, design and maintenance applications began to receive increasing attention, with notable growth in the last five years.Compared with the general trend of AI applications, DRL has shown a more focused and rapid emergence in recent years. In parallel, DRL has emerged as a rapidly growing subset within the AI research landscape. While its early presence was limited, the recent surge reflects its expanding role across all three domains. Particularly, DRL-based control strategies dominate, while applications in design and maintenance are also gaining momentum.

Despite the growing integration of DRL techniques in power electronics, comprehensive reviews specifically addressing DRL applications in PECs remain limited. Several existing reviews in the literature have addressed related topics, but none offer an exhaustive analysis of DRL's role in PEC systems. In [14], the metaheuristic methods for stochastic optimization for power quality and waveform, circuit design, and control tuning are reviewed. It focuses on the optimization tasks only. The details of neural network (NN) in industrial applications are presented in [15] with the design of network structure, training methods, and application considerations. It covers a broad scope of engineering applications beyond power electronics. In [16], a comprehensive review is given on the applications of NN in power electronics. Several specific examples of control and system identification are detailed. Nevertheless, other AI techniques, such as fuzzy logic, metaheuristic methods, etc., have not been discussed. Although these techniques are further discussed in [17], it emphasizes on illustrative examples while an in-depth analysis of AI algorithms is not provided. In [16], an intensive discussion of metaheuristic methods for MPPT in photovoltaic (PV) systems is presented. In [18], the AI techniques applied to PV systems are reviewed, which focus on the specific PV applications only.[19]

As a result, there is a need for a comprehensive review of DRL algorithms and their applications in power electronics. From a life-cycle perspective, this article aims to address this gap by systematically reviewing the published research on DRL techniques for PEC systems. The key contributions of this article are as follows.
\begin{itemize}
  \item [1)]A systematic classification of DRL algorithms used in PECs, highlighting the relationships between different algorithms, their functional roles, and associated applications.
  \item [2)]A comparative performance analysis of DRL methods across different PEC tasks, identifying strengths, weaknesses, and areas of improvement.
  \item [3)]A discussion on the deployment of DRL techniques in real PEC hardware, examining challenges related to hardware integration, real-time constraints, and scalability for practical applications.
  \item [4)]An exploration of future research directions, including potential breakthroughs in DRL algorithmic design and its integration into PECs for improved efficiency, robustness, and autonomy.
\end{itemize}

The remainder of the article is organized as follows. Section II presents an overview of DRL algorithms and their functional roles in PECs. Sections III–V cover the applications of DRL in design optimization, control, and maintenance, respectively. Section VI provides an outlook on the future of DRL in PECs, and Section VII concludes the article.
