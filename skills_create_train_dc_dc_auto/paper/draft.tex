...
\textbf{LLM Fixed (Ours)} & $\mathbf{-0.0370 \pm 0.0004}$ & \textbf{0.07} & \textbf{0.21} & \textbf{972} \\
\textbf{LLM Event (Ours)}  & $-0.0368 \pm 0.0005$ & 0.05 & 0.19 & 985 \\
\bottomrule
\end{tabular}
\end{table}
...
strategy achieves the same performance 15\% faster than random search (972\,s vs.\
1,150\,s), while the LLM event-triggered variant achieves equivalent performance
while invoking the LLM only when the reward plateau detector fires,
rather than on a fixed clock, reducing unnecessary API calls during
periods of steady improvement.
...
First, the 200-episode training horizon used in the ablation is shorter than
typical production RL training, and convergence differences may be more
pronounced at larger scales (500+ episodes); expanded multi-trial experiments
are ongoing and will be reported in the final version.
...
reduction. Our event-triggered intervention mode mitigates this by reducing
unnecessary LLM calls during periods of steady improvement.
