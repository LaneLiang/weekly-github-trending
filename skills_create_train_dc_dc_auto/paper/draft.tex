...
$^{*}$Corresponding author: lianghan@seu.edu.cn
}
\end{flushleft}
...
We further introduce an event-triggered intervention mechanism that invokes the LLM
only when training plateaus, reducing unnecessary API calls.
In a four-strategy ablation across 200 episodes (3 trials each), LLM-driven and
random-search strategies converge to essentially identical final performance
($\approx$$-$0.037 reward, $<$0.1\% voltage error), demonstrating that the SAC agent
learns the task robustly under diverse meta-optimizers. Bayesian optimization, by
contrast, diverged on one of three trials, exposing a fundamental brittleness of
sample-efficient surrogates in noisy RL settings. The event-triggered plateau
detector produced the lowest trial-to-trial variance ($\pm$0.0003), suggesting that
targeted intervention improves reproducibility. Under domain-randomized component
aging (ESR, inductance, capacitance, input voltage, and load all perturbed
simultaneously), the system improves reward 5.9$\times$ over 100 episodes, reducing
voltage error from 8.5\,V to 0.19\,V. The LLM exhibits emergent staged coaching
behavior---restrained observation, then exploration promotion, then
exploitation-focused fine-tuning---without explicit programming. These results
indicate that LLM-based continuous meta-optimization, while not yet outperforming
simpler baselines on this benchmark, offers interpretable reasoning, reduces
trial-to-trial variance, and opens a new architectural direction for automated
power converter synthesis.
\end{abstract}
...
\begin{enumerate}
    ...
    \item A four-strategy ablation study (3 trials $\times$ 200 episodes each)
    comparing random search, Bayesian optimization with GP surrogates, LLM
    fixed-interval, and LLM event-triggered meta-optimization, revealing that
    (a) LLM-guided training is competitive with traditional methods, (b) BO
    surrogates can be brittle in noisy RL settings, and (c) plateau-aware
    intervention reduces trial-to-trial variance;
    \item A plateau-detection mechanism for event-triggered LLM intervention that
    replaces fixed-interval scheduling with need-driven coaching, reducing
    unnecessary API calls during periods of steady improvement;
...
\textbf{Event-Triggered Intervention.}
In addition to fixed-interval intervention (every 50 episodes), we implement
a plateau-detection mechanism that triggers LLM consultation only when reward
improvement stalls.
A sliding window of the last $W = 20$ episode rewards is maintained; if the
relative improvement between the older and recent halves of the window falls
below a threshold $\theta = 0.02$ for $P = 5$ consecutive checks, a plateau
is declared and the LLM is invoked.
Minimum and maximum interval guards ($T_{\min} = 10$, $T_{\max} = 100$ episodes)
prevent both over-frequent and excessively rare interventions.
This event-triggered mode replaces the fixed schedule with a need-driven policy,
addressing a key limitation of interval-gated meta-optimization.

\subsection{Multi-Objective Reward Function}

The reward function combines six performance objectives:
\begin{equation}
...
Two additional metrics (transient recovery time $r_{\text{tr}}$ and phase margin
$r_{\text{pm}}$) are monitored and reported to the LLM but not yet incorporated
into the scalar reward, serving as reserved expansion channels for future
...
We compare four meta-optimization strategies across 200 training episodes
...
\begin{enumerate}
    ...
    \item \textbf{LLM Fixed-Interval (Ours)}: DeepSeek-Chat receives training
    summaries and returns structured JSON hyperparameter/weight suggestions
    at fixed 50-episode intervals.
    \item \textbf{LLM Event-Triggered (Ours)}: Same LLM backend as (3), but
    interventions are triggered by plateau detection rather than a fixed clock,
    reducing unnecessary API calls during periods of steady improvement.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig2_convergence.png}
\caption{Convergence comparison across four meta-optimization strategies.
...
and LLM API call count for the two LLM-driven variants.}
\label{fig:convergence}
\end{figure}

\begin{table}[H]
\centering
\caption{Ablation study results: four meta-optimization strategies compared over 200 episodes
with 3 independent trials each. Mean $\pm$ std reported.}
\label{tab:ablation}
\begin{tabular}{lccccc}
\toprule
\textbf{Strategy} & \textbf{Final Reward} & \textbf{Vo Error \%} & \textbf{Ripple \%} & \textbf{Time (s)} & \textbf{LLM Calls} \\
\midrule
Random Search      & $-0.0370 \pm 0.0006$ & $0.05 \pm 0.01$ & $0.18 \pm 0.01$ & $946 \pm 54$ & $4 \pm 0$ \\
Bayesian Opt (GP)  & $-0.0881 \pm 0.0697$ & $2.48 \pm 3.43$ & $2.22 \pm 2.58$ & $1006 \pm 33$ & $4 \pm 0$ \\
LLM Fixed-Interval & $-0.0369 \pm 0.0004$ & $0.07 \pm 0.04$ & $0.17 \pm 0.02$ & $1026 \pm 54$ & $4 \pm 0$ \\
LLM Event-Triggered& $-0.0368 \pm 0.0003$ & $0.06 \pm 0.05$ & $0.19 \pm 0.03$ & $1084 \pm 14$ & $7 \pm 1$ \\
\bottomrule
\end{tabular}
\end{table}

Three of the four strategies---random search, LLM fixed-interval, and LLM
event-triggered---converge to essentially indistinguishable final performance
($\approx$$-0.037$ reward, $<$0.1\% voltage error, $<$0.2\% ripple), indicating
that the SAC agent learns the Buck converter task robustly regardless of the
meta-optimization approach, provided the hyperparameter perturbations remain
within reasonable bounds.

Bayesian optimization, by contrast, failed catastrophically on one of three
trials (final reward $-$0.177, voltage error 7.3\%; the other two trials
converged normally), producing a wide performance spread ($\pm$0.07).
This failure mode arises because the GP surrogate model, operating on only
4--5 observations, can propose configurations that lie far from the safe
region---and once training destabilizes, subsequent surrogate updates
compound the error. This finding highlights a fundamental brittleness of
sample-efficient black-box optimizers in the RL meta-optimization setting,
where reward surfaces are noisy and highly non-stationary.

The LLM event-triggered strategy invoked the LLM more frequently (7$\pm$1 calls
vs.\ 4$\pm$0 for fixed-interval), demonstrating that the plateau detector
identifies training stagnation beyond the fixed schedule. That this produces
the lowest variance across trials ($\pm$0.0003, roughly half that of random
search) suggests that targeted, plateau-aware intervention may improve
reproducibility even when aggregate reward means are similar.

We further conducted a 3-trial validation at the 50-episode horizon
(Supplementary Table~S1).
At this shorter horizon, LLM-driven strategies underperform non-LLM baselines
(random: $-$0.0427$\pm$0.0007; LLM fixed: $-$0.0593$\pm$0.0045), and the
event-triggered variant makes zero LLM calls, as the plateau detector correctly
identifies that the agent is still improving rapidly. This finding corroborates
the ``restrained observation'' phase identified in our coaching behavior
analysis (Section~4.4): premature LLM intervention provides no benefit and may
mildly disrupt training, while the plateau detector successfully defers action
until meaningful convergence stagnation occurs.
...
Our 200-episode multi-trial ablation reveals a more nuanced picture than a simple
``LLM is better'' narrative. Three strategies---random search, LLM fixed-interval,
and LLM event-triggered---converge to statistically indistinguishable final
performance ($\approx$$-$0.037 reward, $<$0.1\% voltage error). This indicates that
the SAC agent learns the Buck converter control task robustly under many
hyperparameter configurations, and that no meta-optimizer---LLM-based or
otherwise---conferred a decisive edge at this training horizon on this benchmark.

The fourth strategy, Bayesian optimization with GP surrogates, failed on one of
three trials, producing final voltage errors exceeding 7\%. This failure mode is
instructive: when operating with only 4--5 observations in a noisy,
non-stationary reward landscape, the GP's acquisition function can propose
configurations far from the safe manifold. Once training destabilizes, subsequent
surrogate updates reinforce the error rather than correcting it. This highlights
an underappreciated fragility of sample-efficient black-box optimizers in
RL meta-optimization settings, where reward surfaces are significantly more
hostile than in standard hyperparameter optimization benchmarks.

That said, the LLM-driven strategies offer two advantages not captured by raw
reward means. First, the event-triggered variant achieved the lowest
trial-to-trial variance ($\pm$0.0003 vs.\ $\pm$0.0006 for random search), suggesting
that plateau-aware, targeted intervention may improve reproducibility---a property
of practical value when deploying trained controllers to hardware. Second, the
LLM produces human-readable reasoning traces at each intervention, providing
interpretability that random search and GP surrogates cannot offer. This
transparency may accelerate expert debugging and trust-building in
safety-critical power applications.
...
perturbation indicates that the policy generalizes across the aging envelope
rather than memorizing a single operating point.

Several limitations merit discussion.
First, our main ablation uses 200 episodes; while this is sufficient for
the SAC agent to converge on the Buck task, it may be insufficient to reveal
meta-optimizer advantages that manifest only at longer horizons or on harder
problems. Second, LLM inference introduces per-intervention latency (2--5\,s)
and API cost, which partially offsets automation benefits. The event-triggered
mode mitigates premature calls---at 50 episodes it correctly made zero calls
across all trials---but at 200 episodes it actually increased the call count
(7 vs.\ 4), as the plateau detector identified more stagnation events during
extended training. Third, our domain-randomization ranges, while grounded in
aging physics, have not been validated against hardware degradation
measurements. Fourth, the current reward function uses a simplified efficiency
proxy (conduction loss) rather than a full switching-loss model. Fifth, and
most critically, the current benchmark task (Buck CCM regulation) may simply
be too easy for meta-optimization to matter: the SAC agent converges reliably
under almost any reasonable hyperparameter configuration. More challenging
benchmarks---multi-output converters, wide-range DCM/CCM transitions,
simultaneous topology selection---are needed to stress-test the meta-optimizer
and reveal where LLM guidance provides genuine advantage over random search.
...
A four-strategy ablation study (3 trials $\times$ 200 episodes) shows that LLM-guided
strategies are competitive with---though not yet superior to---random search on the
Buck converter benchmark, while Bayesian optimization with GP surrogates proved
unreliable, diverging on one of three trials.
The event-triggered plateau detector achieved the lowest trial-to-trial variance
($\pm$0.0003), suggesting that targeted intervention improves reproducibility.
...
These findings suggest that LLM-based continuous meta-optimization is a
scientifically interesting architectural direction for automated power
converter control synthesis, though its quantitative advantage over simpler
methods remains to be demonstrated on harder benchmarks.
The architecture is topology-agnostic (demonstrated on both Buck and Boost)
and mode-agnostic (CCM and DCM handled transparently).
The event-triggered intervention mechanism introduced here addresses the
cost--benefit trade-off inherent in LLM-based meta-optimization by aligning
LLM calls with actual training need: suppressing intervention during periods
of rapid improvement and issuing targeted suggestions when convergence stalls.
Future work should pursue hardware-in-the-loop validation with real power
converters and extension to more complex topologies such as LLC resonant
converters and multiphase interleaved designs.

\section*{Data Availability}
The complete source code, configuration files, experimental logs, and test
suite (50 tests, all passing) are available at
...
Current reward weights:
  w_ev=1.0, w_vr=1.0, w_eff=1.0, w_os=1.0,
  w_us=1.0, w_tr=1.0, w_ts=1.0
...
Supplementary materials including full hyperparameter tables, evaluation metric
definitions, expanded experimental protocol, and additional waveform analyses
are available in the accompanying \texttt{supplementary.pdf}.

\section*{Acknowledgments}
We thank the DeepSeek team for providing API access to DeepSeek-Chat.
This work was supported by the National Natural Science Foundation of China.

\section*{Author Contributions}
L.L. conceived the study, developed the methodology, implemented the software,
conducted all experiments, analyzed the results, and wrote the manuscript.
