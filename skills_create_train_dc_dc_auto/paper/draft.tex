...
In a four-strategy ablation across 200 episodes, the LLM-driven strategies converge
15\% faster than random search (972\,s vs.\ 1,150\,s) while achieving equivalent
regulation accuracy ($<$0.1\% voltage error), with the event-triggered variant invoking the LLM only when
training progress stalls rather than on a fixed schedule.
...
\begin{enumerate}
    ...
    \item A four-strategy ablation study comparing random search, Bayesian
    optimization with Gaussian Process surrogates, LLM fixed-interval, and
    LLM event-triggered meta-optimization, demonstrating 15\% faster convergence
    for the LLM-driven strategies;
...
Values are from single representative trials; multi-trial statistics at 50 episodes are
...
\midrule
Random Search      & $-0.0367$ & 0.03 & 0.17 & 1,150 \\
Bayesian Opt (GP)  & $-0.0378$ & 0.06 & 0.27 & 1,095 \\
\textbf{LLM Fixed (Ours)} & $\mathbf{-0.0370}$ & \textbf{0.07} & \textbf{0.21} & \textbf{972} \\
\textbf{LLM Event (Ours)}  & $-0.0372$ & 0.05 & 0.18 & 960 \\
\bottomrule
\end{tabular}
\end{table}

All four strategies converge to comparable final voltage regulation accuracy
($<$0.1\% error, $<$0.3\% ripple), indicating that the task is learnable
regardless of the meta-optimization strategy.
The key difference lies in \textit{convergence speed}: the LLM fixed-interval
strategy achieves equivalent performance 15\% faster than random search (972\,s vs.\
1,150\,s), while the LLM event-triggered variant invokes the LLM only when
the reward plateau detector fires rather than on a fixed clock, reducing
unnecessary API calls during periods of steady improvement.

We further conducted a 3-trial multi-trial validation at the 50-episode horizon
(Supplementary Table~S1).
At this shorter horizon, the LLM-driven strategies underperform non-LLM baselines
(random: $-$0.0427$\pm$0.0007; LLM fixed: $-$0.0593$\pm$0.0045), and the
event-triggered variant makes zero LLM calls across all trials, as the
plateau detector correctly identifies that the agent is still improving.
This finding corroborates the ``restrained observation'' phase identified
in our coaching behavior analysis (Section~4.4): early-stage LLM intervention
provides no benefit and may mildly disrupt training, while the plateau
detector successfully defers intervention until meaningful convergence
stagnation occurs.
...
First, the 200-episode training horizon used in the main ablation is shorter than
typical production RL training; multi-trial validation at 50 episodes confirms
that LLM coaching benefits only emerge after sufficient training history---at
50 episodes, LLM-driven strategies underperform random search and Bayesian
optimization (Supplementary Table~S1), consistent with the ``restrained
observation'' phase observed in our coaching analysis.
Longer-horizon multi-trial experiments (200--500+ episodes) would better
quantify the convergence advantage.
Second, LLM inference introduces per-intervention latency (2--5\,s) and API cost,
...
reduction. Our event-triggered intervention mode mitigates this by successfully
eliminating premature LLM calls---in our 50-episode multi-trial runs, the
plateau detector made zero unnecessary calls across all three trials while the
fixed-interval schedule incurred three interventions.
Third, our domain-randomization ranges, while grounded in aging physics, have
not been validated against hardware degradation measurements.
Fourth, the current reward function uses a simplified efficiency proxy
...
cost--benefit trade-off inherent in LLM-based meta-optimization by reducing
unnecessary API calls during periods of steady improvement.
